...
Teepot is a Rust toolkit for Trusted Execution Environments (TEEs), providing attestation verification, cryptographic operations, and utilities for Intel SGX and TDX. Part of the Matter Labs ecosystem.
...
| Build as CI would | `nix run github:nixos/nixpkgs/nixos-24.11#nixci -- build` |
...
- `crates/teepot-tdx-attest-rs` / `teepot-tdx-attest-sys` — TDX attestation FFI (x86_64-linux only, excluded from workspace on other platforms)
...
- **SPDX headers**: Every source file must have an SPDX license header (`// SPDX-License-Identifier: Apache-2.0` or `BSD-3-Clause` or `MIT`). Enforced by CI.
...
- `crates/teepot-tdx-attest-rs` and `crates/teepot-tdx-attest-sys` are excluded from the workspace on non-x86_64 platforms
